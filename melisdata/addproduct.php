...
if(!isset($_SESSION['loggedin']) || $_SESSION['loggedin']!=true){
    header("location:login.php"); //  echo "success messages"; header("Refresh:3; URL=login.php);
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .wrapper{ width: 360px; height: fit-content; margin: auto; width: 60%; padding: 10px; background-color: #FAFAD2;}
    </style>
</head>
<body>   
    ...
    $proname = $imageurl = $prodesc = $username = NULL;
    $pronameerr = $imageurlerr = $prodescerr = NULL;
    $flag = true;
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["description"])) {
			$prodescerr = "Product Description is required";
			$flag = false;
		} else {
			$prodesc = test_input($_POST["description"]);
		}
        $username = $_SESSION['username'];

        if ($flag) {

			$sql1 = "INSERT INTO product (username, productname, imageurl, description) VALUES ('$username', '$proname', '$imageurl', '$prodesc') ";

			// execute sql insert
			if ($connection->query($sql1) === TRUE) { // procudural or classes (oops)
  ...
  <strong>Error! Unsuccessful TRY AGAIN</strong> 
  <button type="button" class="close" data-dismiss="alert" aria-label="Close">
  </button>
</div>';
        }
...
?>
    ...
    <div class="wrapper">
        ...
        <h4>Welcome Seller <?php echo $_SESSION['username']; ?></h4>
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                    <div class="form-group">
                <label>Product Name</label>
                <input type="text" name="productname" class="form-control" value="" placeholder="Product Name">
            </div>    
            <div class="form-group">
                <label>Image URL</label>
                <input type="text" name="imageurl" class="form-control" value="" placeholder="Image URL">
          </div>
  <div class="form-group">
